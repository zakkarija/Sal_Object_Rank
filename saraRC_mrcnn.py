...
def findMostSalientObject(rois, kernel, dws):
    maxEntropy = 0
    index = 0
    i = 0
    for roi in rois:
        # print("roi", roi, kernel[i], dws)
        entropy = calculateEntropy(roi, kernel[i], dws[i])
        Tup = (i, entropy)
...
def generateSaRa(tex, texSegments, results):
    ...
    rois = results['rois']
    maxObj, indexObj = findMostSalientObject(rois, gaussian1d, dws)
    ...
    objList = printObjects(sortedObjEntropies, rois, tex)
    print("maxObj", maxObj)
...
def getObjectWithRank(saraListOutput, rois, rank, img):
    # sal_tuple is (index, rank)
    for sal_tuple in saraListOutput:
        print(sal_tuple)
        if sal_tuple[1] == int(rank):
            for coords in rois:
...
def returnSARA(inputImg, rank_to_show, results):
    ...
    saraOutput, saraListOutput = generateSaRa(inputImg, texSegments, results)
    if rank_to_show != -1:
        roi = getIndexWithRank(saraListOutput, rank_to_show, inputImg)
        cv2.imshow("roi", roi)
        cv2.waitKey()
...
#
# outS1, saraListS1 = returnSARA(s1, -1)
